...
local Aimbot,SilentAimbot,Trigger,
...
local Window = Tayronne.Utilities.UI:Window({
    Name = "🎃 Tayronne Hub — "..Tayronne.Game,
    Position = UDim2.new(0.05,0,0.5,-248)
        ...
        local VisualsTab = Window:Tab({Name = "Visuals"}) do
            ...
            local LightingSection = VisualsTab:Section({Name = "Lighting",Side = "Right"}) do
                ...
                Callback = function(Bool) if Bool then return end
                    for Property,Value in pairs(Tayronne.Utilities.Misc.DefaultLighting) do
        ...
        local SettingsTab = Window:Tab({Name = "Settings"}) do
            ...
            Callback = Tayronne.Utilities.Misc.ReJoin})
            SettingsTab:Button({Name = "Server Hop",Side = "Left",
            Callback = Tayronne.Utilities.Misc.ServerHop})
            SettingsTab:Button({Name = "Join Discord Server",Side = "Left",
            Callback = Tayronne.Utilities.Misc.JoinDiscord})
            ...
            local CreditsSection = SettingsTab:Section({Name = "Credits",Side = "Right"}) do
                CreditsSection:Label({Text = "This script was made by Tayronne"})
            end
        end
    end
    ...
    Tayronne.Utilities.Misc:SetupWatermark(Window)
    Tayronne.Utilities.Misc:SetupLighting(Window.Flags)
    Tayronne.Utilities.Drawing:SetupCursor(Window.Flags)
    
    Tayronne.Utilities.Drawing:FOVCircle("Aimbot",Window.Flags)
    Tayronne.Utilities.Drawing:FOVCircle("Trigger",Window.Flags)
    Tayronne.Utilities.Drawing:FOVCircle("SilentAim",Window.Flags)
    ...
    Tayronne.Utilities.Misc:NewThreadLoop(0,function()
    ...
    Tayronne.Utilities.Misc:NewThreadLoop(1,function()
    ...
    for Index,Item in pairs(Loot:GetDescendants()) do
        ...
        if Item:IsA("Model") and ItemData then --print(ItemData.Parent.Name)
            Tayronne.Utilities.Drawing:ItemESP(
    ...
    Randoms.ChildAdded:Connect(function(Place)
        if table.find(Places,Place.Name) then --print(Place.Name)
            ...
            if Window.Flags["AR2/ESP/RandomPlaces/Enabled"] then
                Tayronne.Utilities.UI:Notification2({
    ...
    Loot.DescendantRemoving:Connect(function(Item)
        if Item:IsA("Model") then
            Tayronne.Utilities.Drawing:RemoveESP(Item.Parent)
        end
    end)
    Randoms.ChildRemoved:Connect(function(Place)
        Tayronne.Utilities.Drawing:RemoveESP(Place)
    end)
    Vehicles.ChildRemoved:Connect(function(Vehicle)
        Tayronne.Utilities.Drawing:RemoveESP(Vehicle)
    end)
    Zombies.ChildRemoved:Connect(function(Zombie)
        Tayronne.Utilities.Drawing:RemoveESP(Zombie)
    end)
    ...
    for Index,Player in pairs(PlayerService:GetPlayers()) do
        ...
        Tayronne.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
    end
    PlayerService.PlayerAdded:Connect(function(Player)
        Tayronne.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
    end)
    PlayerService.PlayerRemoving:Connect(function(Player)
        Tayronne.Utilities.Drawing:RemoveESP(Player)
    end)