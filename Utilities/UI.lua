...
local function GetConfigs()
    if not isfolder("Tayronne") then makefolder("Tayronne") end
    if not isfolder("Tayronne\\Configs") then makefolder("Tayronne\\Configs") end
    if not isfile("Tayronne\\DefaultConfig.txt") then writefile("Tayronne\\DefaultConfig.txt","") end

    local Configs = {}
    for Index,File in pairs(listfiles("Tayronne\\Configs") or {}) do
        File = File:gsub("Tayronne\\Configs\\","")
        File = File:gsub(".json","")
        Configs[Index] = File
    end
    return Configs
...
local function ConfigsToList()
    ...
    for Index,File in pairs(listfiles("Tayronne\\Configs") or {}) do
        File = File:gsub("Tayronne\\Configs\\","")
        File = File:gsub(".json","")
        local DefaultConfig = readfile("Tayronne\\DefaultConfig.txt")
...
local function InitScreen()
    ...
    --[[if Debug then
        ...
        Tayronne.Utilities.Misc:HideObject(ScreenAsset)
    end]]
...
local function InitWindow(ScreenAsset,Window)
    ...
    function Window:SaveConfig(Name)
        ...
        if table.find(GetConfigs(),Name) then
            Config = HttpService:JSONDecode(readfile("Tayronne\\Configs\\"..Name..".json"))
        end
        for Index,Element in pairs(Window.Elements) do
        ...
        writefile("Tayronne\\Configs\\"..Name..".json",HttpService:JSONEncode(Config))
    end
    function Window:LoadConfig(Name)
        if table.find(GetConfigs(),Name) then
            local DecodedJSON = HttpService:JSONDecode(readfile("Tayronne\\Configs\\"..Name..".json"))
            for Index,Element in pairs(Window.Elements) do
    ...
    function Window:DeleteConfig(Name)
        if table.find(GetConfigs(),Name) then
            delfile("Tayronne\\Configs\\"..Name..".json")
        end
    end
            ...
            local ConfigSection = Tab:Section({Name = "Configs",Side = Side}) do
                ...
                ConfigSection:Button({Name = "Set",Callback = function()
                    if ConfigDropdown.Value and ConfigDropdown.Value[1] then
                        ...
                        writefile("Tayronne\\DefaultConfig.txt",DefaultConfig)
                        ConfigDivider:SetText(
                        "Default Config\n<font color=\"rgb(189,189,189)\">[ "..DefaultConfig.." ]</font>")
                    end
                end})
                ConfigSection:Button({Name = "Clear",Callback = function()
                    writefile("Tayronne\\DefaultConfig.txt","")
